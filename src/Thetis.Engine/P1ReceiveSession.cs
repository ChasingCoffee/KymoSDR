...
public sealed record P1ReceiveOptions(int Port, int FrequencyHz = 14_199_000,
    string Address = "127.0.0.1", ReceiveDemodulation? Demodulation = null);
...
public sealed class P1ReceiveSession : ReceiveSession
{
        ...
        CancellationToken token, Func<int, int>? checkpoint)
    {
        ...
        return OpenSession(directory, new(options.Port, 0, 48000, options.FrequencyHz, options.Address, options.Demodulation),
