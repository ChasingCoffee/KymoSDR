...
internal static class TransportCli
{
    private const string Help = "Usage: Thetis.Headless transport-selftest --native-dir ABSOLUTE_PATH\n100 loopback-only RNet/socket lifecycles. No radio commands, TX or audio devices. Outputs JSON.";
    internal static int Run(string[] args, TextWriter output, TextWriter error, CancellationToken token = default,
        Func<string, CancellationToken, TransportSelfTestResult>? runner = null)
    {
        ...
        try
        {
            ...
            var result = (runner ?? TransportDiagnostics.Run)(args[2], token);
            output.WriteLine(JsonSerializer.Serialize(result, new JsonSerializerOptions
            { WriteIndented = true, PropertyNamingPolicy = JsonNamingPolicy.CamelCase }));
            return result.Passed ? 0 : 1;
        }
        catch (OperationCanceledException) { error.WriteLine("Transport self-test cancelled; owned resources were disposed."); return 130; }
        ...
        { error.WriteLine($"Cannot load a compatible native transport build: {ex.Message}"); return 3; }
        ...
        { error.WriteLine($"Transport self-test failed: {ex.Message}"); return 4; }
    }
}
