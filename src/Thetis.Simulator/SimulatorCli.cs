...
internal static class SimulatorCli
{
    internal const string Help = """
        KymoSDR G2 RX simulator — synthetic Protocol 2 peer, IPv4 loopback ONLY.
        No hardware, native DSP, audio devices or TX emulation.
        ...
        wideband, firmware programming, calibration, RF validation or TX support.
    ...
    internal static async Task<int> RunAsync(string[] args, TextWriter output, TextWriter error, CancellationToken token = default)
    {
        try
        {
            ...
            output.WriteLine(JsonSerializer.Serialize(new { schemaVersion = 1, eventType = "ready", simulator = true,
                name = "KymoSDR G2 RX simulator", address = "127.0.0.1", basePort = simulator.BasePort,
                ddc0Port = simulator.BasePort + 11, ddcCount = 10, toneHz = options.ToneFrequencyHz, transmitSupported = false }, Json));
            output.Flush();
    ...
    {
        if (args.Length == 0 || args[0] != "serve" || args.Length % 2 != 1) throw new ArgumentException("Use serve [options], selftest, or --help.");
        ...
        for (int i = 1; i < args.Length; i += 2)
        {
            ...
            switch (key)
            {
                ...
                default: throw new ArgumentException($"Unknown option {key}. There is no LAN bind or transmit option.");
            }
        }
        options.Validate();
