...
{
	if (create_rnet_checked() != 0) abort();
	bandwidth_monitor_reset();
	/* Legacy device callback wiring remains outside the allocation boundary. */
	switch (pcm->audioCodecId)
	{
	case HERMES: SendpOutboundRx(OutBound); break;
	case ASIO: SendpOutboundRx(asioOUT); break;
	case WASAPI: break;
	}
