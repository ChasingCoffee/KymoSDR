...
M2's initial cross-platform offline gate passes on Windows x64, macOS arm64 and Linux x64; see [CI results](NATIVE_CI_RESULTS.md), [initial local M2 results](M2_NATIVE_RESULTS.md) and [native build instructions](NATIVE_DSP.md). M3a's offline ChannelMaster DSP/pipe lifecycle now passes cross-platform tests with a .NET owner, 100-cycle checks, cancellation/rollback, no-device audio and radio-init argument correction. Its qualification and limits are tracked in [ChannelMaster offline](CHANNELMASTER_OFFLINE.md). Full M3 remains partial: M3b's RNet/socket lifecycle, active-streaming shutdown and longer-run resource/performance qualification are still required; M4–M11 remain pending. No radio streaming or live TX capability has been implemented or tested.
