...
| P2 simulator → native CM/WDSP audio | Loopback-only receive integration passes cross-platform | 119 managed tests, 8 native tests, tone/retune, DDC/rate, cancellation, ASan/UBSan and zero-leak scan | 119 managed tests plus native/CLI tests pass on both; Linux active-path sanitizer/leak checks pass | [Receive boundary](P2_RECEIVE_INTEGRATION.md); hardware RX, P1, spectrum and soak tests still pending. |
| Hardware radio streams / RX1 spectrum | Not started | None | None in new app | M4. |
