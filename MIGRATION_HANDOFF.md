...
- **2026-08-22 — 3D cached-surface rendering (push-rate GPU cap)**: user confirmed CPU/RAM flat while FPS fell with Depth Lines >25 → bottleneck is GPU fill-rate (translucent overdraw), not CPU. Fix: full stack now renders into an offscreen compatible target (`_3dOffscreenRT`, `ID2D1BitmapRenderTarget` via `CreateCompatibleRenderTarget(SizeI,SizeI,SDXPixelFormat(B8G8R8A8, Premultiplied))` created FROM the main RT) only when `_3dBmpDirty` — set at the history-push site — or every 300ms forced refresh (covers knob/setting/band changes); every display frame just `DrawBitmap`s the finished surface onto the main RT (near-free blit). Total 3D rasterisation capped at push rate regardless of line count. Implementation notes: plain `ID2D1RenderTarget` can't SetTarget, hence compatible-RT pattern; all in-method draws route through indirection field `_rt3D` (swapped to offscreen before BeginDraw, restored in finally; EndDraw crash-safe with ulong tags); method zeroes its `nVerticalShift` param and captures `destShift` so all stack geometry is built 0-based inside the surface and composited at `(0,destShift,W,H)`; premultiplied transparent Clear keeps translucent painter's-algorithm stacking visually IDENTICAL to direct drawing (alpha compositing associativity); phase pinned to 0 — rows step at push rate instead of sliding sub-row smooth (accepted tradeoff); teardown disposes RT+bitmap and resets dirty in both device-release paths. Vortice gotchas: `PixelFormat` bare name binds to System.Drawing enum — use existing `SDXPixelFormat` alias; sizes are `Vortice.Mathematics.SizeI`; bitmap property is `.Bitmap` on the bitmap render target. User verified: framerate essentially flat when raising Depth Lines.
