...
- **2026-08-22 — "3D Pan" toolbar button added**: new `OtherButtonId.PAN3D` (appended at END of enum — never insert mid-enum, saved button layouts key off int values) wired through the full MW0LGE other-buttons framework: definition row `(3, 28)` placing it in the Display column directly under "A-Peak", caption "3D Pan"; click dispatch → `DoGeneralSettingAction` → `SetupForm.Display3DPanadapter` (existing wrapper over chkDisplay3DPanadapter, so Setup checkbox + engine `Display.Pan3DEnabled` stay in sync and persistence is free); mirrored in `GetGeneralSetting`, `initGeneralSettings` boot sync, and MeterManager's general-settings snapshot. Files: ucOtherButtonsOptionsGrid.cs, console.cs, MeterManager.cs.
