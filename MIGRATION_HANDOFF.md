...
5. GPU spectrum overlays - peak-hold/annotations/markers drawn through the existing D3D11 pipeline (cheap now that mesh passes exist). **DONE 2026-08-27**: `Display.OverlayMesh.cs` (2D + 3D peak-hold overlay), runtime-verified by user ("its working correct now").
6. Code health splits - MeterManager.cs ~45k lines and display.cs ~14k lines into focused partials/modules. **DXVorticeCompat shim RETIRED 2026-08-27** (sub-item of #6): `DXRectF` replaced with `Vortice.Mathematics.Rect` across MeterManager.cs, display.cs, Display.OverlayMesh.cs, Display.Pan2DMesh.cs; shim file deleted + its csproj Compile item removed. D2D RawRectF's implicit Rect op made FillRectangle/PushAxisAlignedClip/FillOpacityMask/RoundedRectangle.Rect work with zero conversion edits; DrawText/DrawBitmap bound native Rect overloads (12 four-arg DrawBitmap calls got a `, null` source-rect arg). MeterManager uses `using Rect = Vortice.Mathematics.Rect;` (avoids the Size/Color namespace clash); display.cs swapped its `using RectangleF = Thetis.DXRectF;` to `Vortice.Mathematics.Rect`. Manual fixes: 7x `Contains(Point)`->`Contains(x,y)` (clickRect), 2x int-based `Rect.Size`->direct `.Width/.Height` (rectBB 40065/40173), 17x `new DXRectF()`->`default`, 289x type-sub. Clean x64 Release Rebuild (exit 0, no errors). Remaining of #6: the source-file split itself.
...
Top picks if only two get done first: #1 (runtime log toggle) and #7 (CI releases) - low effort, recurring value. **Update 2026-08-25**: #1, #2, #3, #7, #8 done. Remaining: #4 (GPU default), #6 (code splits).
