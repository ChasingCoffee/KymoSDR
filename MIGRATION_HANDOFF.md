...
**NEXT SESSION START HERE — GPU fallback scoping facts (gathered 2026-08-22, nothing implemented yet)**:
- Only DX init call site: display.cs ~1262 in the `displayTarget` setter → `initDX2D(DriverType.Hardware, _display_adaptor)`. No WARP retry exists; on init failure the catch (~3735) shows a MessageBox and calls ShutdownDX2D.
- `DX2Adaptors()` at display.cs ~3450 enumerates adapters (used for adaptor preference); device/swapchain teardown block ~3390–3415.
- The legacy non-DX2 GDI draw path is GONE — D2D/Vortice is the only renderer, so "D2DCpu" tier = current line-based renderer (keep it maintained per rule 1), "WarpSoftware" tier = new WARP-backed init attempt.
- Suggested first slice: render-path enum + init chain HW→WARP→error, "Force CPU rendering" setup checkbox persisted via existing display options plumbing, startup ErrorLog.txt line with chosen path + adapter name + feature level.
